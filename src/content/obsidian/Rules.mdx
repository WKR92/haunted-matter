...
- Each object is searched with appropriate tests, which may advance the **Alarm Clock**. If an object requires more than one roll, each roll may advance the clock. Example links between tests and actions:
  - Traps — resolved according to their rules.
  ...
  - Secrets:
    - **perception** or **investigation**, with the **GM** asking **P** whether they risk such a test,
    - then an additional appropriate test if the find is hard to access (e.g. pulling out a loose brick — **nimble hands**). Hidden things should, by default, be hard to access.
...
- **Objects**:
  - Desk — Information + Trap (poisoned needle in drawer) → letter about a gold convoy.
  - Shelf — Information — **investigation** → report ledger with names of spies.
  - Chest — Loot + Trap (blade cutting hands).
  - Wardrobe — Loot — **perception** → guard uniform (**advantage** to impersonation).
  - Cache — 2 × Loot — question to **P**: do you risk **perception**? — loose floorboard — **athletics** — roll twice on the **Loot Table**.
