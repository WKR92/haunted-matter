...
- Use this when resolving how the whole group handled a shared action that requires a single **trait** (e.g. everyone sneaks past the guards, everyone climbs a rope, everyone blends into a crowd), or different **traits** (e.g. organizing an event, setting up a trap).
...
- The group must beat a fixed threshold of **(2 × number of players) + 1** for the test to succeed.
- After seeing S, the group may lower the threshold by spending **fatigue**:
    - 4 **fatigue** lowers the threshold by 1 point
    - the threshold can be lowered to a minimum of **(2 × number of players) − 2**
    - **fatigue** can be split between players in any way, but a single player may spend at most 4 **fatigue** on this test
...
- The breakdown below shows how often the group succeeds on group rolls in a typical situation (**5 players**, each rolling **3 dice**), assuming optimal use of fatigue:
    - S ≥ 11 → success without fatigue: ~44%
    - S = 10 → success for **4 fatigue**: ~16%
    - S = 9 → success for **8 fatigue**: ~14%
    - S = 8 → success for **12 fatigue**: ~11%
    - S ≤ 7 → failure (the threshold cannot be lowered further): ~15%
...
4. **Descriptions:** each **P** narrates 1-2 sentences about what happened to them — how they found something, got hurt, spoke with someone, etc.
...
- For chases, the base success threshold of the group test is reduced to **(2 × number of players)**.
