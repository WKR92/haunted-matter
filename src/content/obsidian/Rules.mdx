...
- **Buying effects with successes** — if you roll more than 1 success, you may convert each of them into an extra effect (instead of dealing damage), such as a free maneuver, a move, using an item, etc. The first extra success can be converted for free; converting each additional one costs one fatigue.
    - Example: you roll 5, 4, 4, 2 to attack. You decide to deal damage with the first two successes (5 and 4) and convert the remaining success — which would normally deal damage — into a shove for free. Or you deal damage only with the 5, convert the first 4 into a shove for free, and then convert the next one, for 1 fatigue, into a knockdown.
    - You can never convert an extra success into an attack.
