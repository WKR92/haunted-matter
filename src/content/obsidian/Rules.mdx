...
- **Choosing the Enemy’s Target**:
  - Choose the target that best fits the narrative, or:
    - Determine the list of possible targets (those the enemy can realistically reach).
    - Arrange them according to the current turn order.
    - Use the first **non-main die** from the left.
      - If there is only one die, the last in the order is always hit.
    - If the result is higher than the number of targets, loop back to the start of the list and count forward again.
      - If the **main die** shows 5–6, the enemy attacks the easiest/most dangerous target.
      - If the enemy does not choose the most optimal target, interpret it narratively as the chaos of battle — someone stepped into their path, they lost balance, hesitated, or misjudged the situation.
